...
mots = ['emmental', 'conte', 'chevre', 'cammenbert', 'brie']
...
font_path = "Assets/Font/catfiles/Catfiles.ttf"  # Chemin de la police
font_size = 16
...
# Fonction pour placer les mots dans le tableau de manière procédurale
def placer_mots_procéduralement(mots, tableau):
    for mot in mots:
        placer_mot_de_façon_aleatoire(mot, tableau)

# Fonction pour placer un mot de manière aléatoire dans le tableau sans chevauchement
def placer_mot_de_façon_aleatoire(mot, tableau):
    mot_placé = False
    while not mot_placé:
        direction = random.choice(['horizontal', 'vertical'])
        if direction == 'horizontal':
            x = random.randint(0, len(tableau[0]) - len(mot))
            y = random.randint(0, len(tableau) - 1)
            if mot_peut_etre_place_horizontalement(mot, x, y, tableau):
                placer_mot_horizontalement(mot, x, y, tableau)
                mot_placé = True
        else:
            x = random.randint(0, len(tableau[0]) - 1)
            y = random.randint(0, len(tableau) - len(mot))
            if mot_peut_etre_place_verticalement(mot, x, y, tableau):
                placer_mot_verticalement(mot, x, y, tableau)
                mot_placé = True

# Fonction pour vérifier si un mot peut être placé horizontalement à une position donnée
def mot_peut_etre_place_horizontalement(mot, x, y, tableau):
    for i, lettre in enumerate(mot):
        if tableau[y][x + i] != ' ' and tableau[y][x + i] != lettre:
            return False
    return True

# Fonction pour placer un mot horizontalement à une position donnée
def placer_mot_horizontalement(mot, x, y, tableau):
    for i, lettre in enumerate(mot):
        tableau[y][x + i] = lettre

# Fonction pour vérifier si un mot peut être placé verticalement à une position donnée
def mot_peut_etre_place_verticalement(mot, x, y, tableau):
    for i, lettre in enumerate(mot):
        if tableau[y + i][x] != ' ' and tableau[y + i][x] != lettre:
            return False
    return True

# Fonction pour placer un mot verticalement à une position donnée
def placer_mot_verticalement(mot, x, y, tableau):
    for i, lettre in enumerate(mot):
...
# --------------------------------------------
# Fonction pour créer un cadre arrondi
...
# Position initiale du cadre
frame_x = (window_width - frame_width) // 2
frame_y = (window_height - frame_height) // 2

# Ajouter le cadre avec coins arrondis
canvas.create_image(frame_x + frame_width // 2, frame_y + frame_height // 2, image=frame_photo, tags="frame")


# Liste de mots à inclure dans le tableau
mots = ['emmental', 'conte', 'chevre', 'cammenbert', 'brie' ] # 'gouda', 'mimolette', 'roquefort', 'raclette', 'Valencay']
words = ['emmental', 'conte', 'chevre', 'cammenbert', 'brie' ] # 'gouda', 'mimolette', 'roquefort', 'raclette', 'Valençay']

"""
mots = ['Pommes', 'Banane', 'Mangue', 'Framboise', 'Pêche', 'Abricot', 'Fraise', 'Raisin']
words = ['Pommes', 'Banane', 'Mangue', 'Framboise', 'Pêche', 'Abricot', 'Fraise', 'Raisin']
"""
cell_width = 30
cell_height = 30
text_color = "black"
...
# Appeler la fonction pour placer les mots dans le tableau
placer_mots_procéduralement(mots, tableau)
